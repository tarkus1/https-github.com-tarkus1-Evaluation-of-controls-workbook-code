Attribute VB_Name = "BuildControlsWithRedundant"
...
Sub FilterBPNCEs()
    ...
    wsNCE.Range("NCESub[#All]").AdvancedFilter Action:=xlFilterCopy, CriteriaRange:= _
        wsNCE.Range("NCE_BP"), CopyToRange:=wsNCE.Range("s1:x1"), Unique:=True
...
Sub FilterClientControls()
    ...
    wsClient.Range("ClientControls[#All]").AdvancedFilter Action:=xlFilterCopy, _
        ...
        "CC_Extract"), Unique:=True
    

End Sub
Sub RemoveRedundantControls()
    
    Set wsClient = ActiveWorkbook.Worksheets("Client Controls")
    Set wsNCE = ActiveWorkbook.Worksheets("NCE Component")

    'Debug.Print "in RemoveRedundantControls for the " & wsSheet.Name
    
    Set newCriteria = wsClient.Range(Range("CC_Criteria"), Range("CC_Criteria").End(xlDown))

    wsClient.Range("ClientControls[#All]").AdvancedFilter Action:=xlFilterCopy, _
        CriteriaRange:=newCriteria, CopyToRange:=wsClient.Range("Redundant"), Unique:=True
    
    'is there any redundant controls? if not exit.
    
    If (wsClient.Range("Redundant").Offset(1) = "") Then Exit Sub
        
    'delete those NCE_components from the BP_NCEs.
    wsClient.Activate
    
    Set delRange = wsClient.Range(Range("Redundant").Offset(1, 0), Range("Redundant").End(xlDown))
    ...
    'delete the rows with the redundant NCE_Components
    
    For Each delN In delRange
        On Error GoTo ERRORHANDLER

        Debug.Print "Checking for ", delN
               
        delrow = Application.WorksheetFunction.Match(delN, wsNCE.Range("BP_NCEs[NCE Component]"), 0)
    
        Debug.Print "the row ", delrow
        
        wsNCE.Activate
        
        wsNCE.ListObjects("BP_NCEs").DataBodyRange(delrow).Delete
...
Sub CopyToTable()
    ...
    'need to filter out redundant controls to the client's controls
    ...
    wsClient.ListObjects.Add(xlSrcRange, wsClient.Range("CC_Extract").CurrentRegion, , xlYes).Name _
        = "BP_ClientControls"
